...
func (userQuery *UserQuery) CreateUserQuery() graphql.Fields {
	// hobbyQuery := userQuery.HobbyQuery.CreateHobbyQuery()
	return graphql.Fields{
		...
		"Hobby": &graphql.Field{
			...
			Resolve: func(p graphql.ResolveParams) (interface{}, error) {
				...
				v, _ := id.(int)
				fmt.Printf("fetching post with id: %d", v)
				u, err := userQuery.HobbyUsecase.GetHobby(&entities.Hobby{
					ID:   0,
					Name: "野球",
				})
				if err != nil {
